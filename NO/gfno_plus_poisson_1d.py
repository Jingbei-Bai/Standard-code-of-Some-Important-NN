...
class GFNOPlusBlock(nn.Module):
    def __init__(self, width, modes, dilation):
        super().__init__()
        self.spec = SpectralConv1d(width, width, modes)
        self.local = nn.Conv1d(width, width, kernel_size=3, padding=dilation, dilation=dilation)
    ...
    def forward(self, x):
        ...
        return torch.nn.functional.gelu(g * s + (1.0 - g) * l + self.skip(x))


class GFNOPlus1d(nn.Module):
    def __init__(self, modes=20, width=96, n_layers=6):
        super().__init__()
        self.fc0 = nn.Linear(2, width)
        self.blocks = nn.ModuleList(
            [GFNOPlusBlock(width, modes=modes, dilation=(i % 3) + 1) for i in range(n_layers)]
        )
...
def train_gfno_plus_poisson_1d(
    n_train=2600,
    n_test=300,
    n_grid=256,
    k_max=1,
    ...
    c_train = sample_coeffs(n_train, k_max=k_max, seed=81)
    c_test = sample_coeffs(n_test, k_max=k_max, seed=82)
    ...
    model = GFNOPlus1d(modes=modes, width=width, n_layers=n_layers).to(device)
    ...
    plt.plot(x_grid, pred[idx], "r--", lw=2, label="gFNO+")
    plt.xlabel("x")
    plt.ylabel("u(x)")
    ...
    plt.legend()
    plt.tight_layout()
    fig_path = os.path.join(out_dir, "gfno_plus_poisson_1d_example.png")
    ...
    plt.title("gFNO+ 1D Poisson Training Loss")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    loss_fig = os.path.join(out_dir, "gfno_plus_poisson_1d_loss.png")
...
if __name__ == "__main__":
        ...
        print("device:", device)
        out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "outputs"))
        train_gfno_plus_poisson_1d(out_dir=out_dir, device=device)
