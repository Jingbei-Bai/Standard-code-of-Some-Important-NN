...
class HaarWaveletConv1d(nn.Module):


    def __init__(self, channels):
        super().__init__()
        self.aa = nn.Conv1d(channels, channels, 1)
        self.ad = nn.Conv1d(channels, channels, 1)
        self.da = nn.Conv1d(channels, channels, 1)
        self.dd = nn.Conv1d(channels, channels, 1)
    ...
    def forward(self, x):
        ...
        a_new = self.aa(a) + self.ad(d)
        d_new = self.da(a) + self.dd(d)

        even_new = (a_new + d_new) / s2
        odd_new = (a_new - d_new) / s2
...
class WNO1d(nn.Module):
    def __init__(self, width=64, n_layers=4):
        super().__init__()
        self.fc0 = nn.Linear(2, width)
        self.wav_layers = nn.ModuleList([HaarWaveletConv1d(width) for _ in range(n_layers)])
    ...
    def forward(self, f_in):
        ...
        for wav, w in zip(self.wav_layers, self.w_layers):
            x = torch.nn.functional.gelu(wav(x) + w(x))
...
def train_wno_poisson_1d(
    ...
    c_train = sample_coeffs(n_train, k_max=k_max, seed=31)
    c_test = sample_coeffs(n_test, k_max=k_max, seed=32)
    ...
    model = WNO1d(width=width, n_layers=n_layers).to(device)
    ...
    plt.plot(x_grid, pred[idx], "r--", lw=2, label="WNO")
    plt.xlabel("x")
    plt.ylabel("u(x)")
    ...
    plt.legend()
    plt.tight_layout()
    fig_path = os.path.join(out_dir, "wno_poisson_1d_example.png")
    ...
    plt.title("WNO 1D Poisson Training Loss")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    loss_fig = os.path.join(out_dir, "wno_poisson_1d_loss.png")
...
if __name__ == "__main__":
        ...
        print("device:", device)
        out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "outputs"))
        train_wno_poisson_1d(out_dir=out_dir, device=device)
