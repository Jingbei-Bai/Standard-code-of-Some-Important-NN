...
class DecomposedSpectralConv1d(nn.Module):


    def __init__(self, in_channels, out_channels, low_modes=12, mid_modes=12):
        super().__init__()
        ...
        self.w_low = nn.Parameter(
            scale * torch.randn(in_channels, out_channels, low_modes, dtype=torch.cfloat)
        )
        self.w_mid = nn.Parameter(
            scale * torch.randn(in_channels, out_channels, mid_modes, dtype=torch.cfloat)
        )
    ...
    def forward(self, x):
        ...
        m_low = min(self.low_modes, n_freq)
        out_ft[:, :, :m_low] = torch.einsum(
            "bim,iom->bom", x_ft[:, :, :m_low], self.w_low[:, :, :m_low]
        )

        start_mid = min(self.low_modes, n_freq)
        max_mid = max(0, n_freq - start_mid)
        m_mid = min(self.mid_modes, max_mid)
        if m_mid > 0:
            out_ft[:, :, start_mid : start_mid + m_mid] = torch.einsum(
                "bim,iom->bom",
                x_ft[:, :, start_mid : start_mid + m_mid],
                self.w_mid[:, :, :m_mid],
            )
...
class DFNOPlus1d(nn.Module):
    def __init__(self, low_modes=12, mid_modes=12, width=96, n_layers=5):
        super().__init__()
        self.fc0 = nn.Linear(2, width)
        self.spec_layers = nn.ModuleList(
            [
                DecomposedSpectralConv1d(width, width, low_modes=low_modes, mid_modes=mid_modes)
        ...
        self.skip_layers = nn.ModuleList([nn.Conv1d(width, width, 1) for _ in range(n_layers)])
    ...
    def forward(self, f_in):
        ...
        for spec, local, skip in zip(self.spec_layers, self.local_layers, self.skip_layers):
            x = torch.nn.functional.gelu(spec(x) + local(x) + skip(x))
...
def train_dfno_plus_poisson_1d(
    n_train=2600,
    n_test=300,
    n_grid=256,
    k_max=1,
    ...
    c_train = sample_coeffs(n_train, k_max=k_max, seed=71)
    c_test = sample_coeffs(n_test, k_max=k_max, seed=72)
    ...
    model = DFNOPlus1d(
        low_modes=low_modes, mid_modes=mid_modes, width=width, n_layers=n_layers
    ).to(device)
    ...
    pred_n, _ = evaluate_model(model, f_test_n, (u_test - u_mean) / u_std, device=device)
    ...
    plt.plot(x_grid, pred[idx], "r--", lw=2, label="dFNO+")
    plt.xlabel("x")
    plt.ylabel("u(x)")
    ...
    plt.legend()
    plt.tight_layout()
    fig_path = os.path.join(out_dir, "dfno_plus_poisson_1d_example.png")
    ...
    plt.title("dFNO+ 1D Poisson Training Loss")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    loss_fig = os.path.join(out_dir, "dfno_plus_poisson_1d_loss.png")
...
if __name__ == "__main__":
        ...
        print("device:", device)
        out_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "outputs"))
        train_dfno_plus_poisson_1d(out_dir=out_dir, device=device)
